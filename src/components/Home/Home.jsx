...
const Home = () => {
  ...
  return (
    <div>
      ...
      <div className="mb-10">
        <h2 className="text-center font-bold text-4xl mt-40">
          Job Category List
        </h2>
      ...
      <div className="grid sm:grid-cols-1 lg:grid-cols-2 mt-10 lg:ml-80">
        {items.slice(0, show).map((item) => (
          <FeaturedJobs key={item.id} item={item}>
            {" "}
          </FeaturedJobs>
        ))}
      </div>
      <div className="text-center mb-8">
        <button onClick={loadMore} className="btn btn-outline btn-success">Show All</button>
      </div>
    </div>
  );
};
